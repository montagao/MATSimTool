...
w_title = 'Voltage Divider Characteristics';
w_width = 800;
w_height = 800;

%properties of the main plot
p_title = 'Output voltage vs R2/R1';
y_label = 'Vo';
x_label = 'R2/R1';
legendlabels = {'V2','V1', 'Vs'};
...
fig_path = 'voltagedivgraphic.png';
...
c_names = cellstr(char('Vs [V]', 'Rs [Ohms]'));
c_values = [1 50];
...
v_names = cellstr(char('R2/R1 Max' ));
v_lims = [ 0.1 50  ];
isXScale = true;
...
user_fn_filenames = cellstr(char('voltagediveqn.m'));

%Give brief description of the simulation 
info = cellstr(char('This plot demonstrates the basic principle of a voltage divider.',...
    'Notice that when R2 is approx 10x greater than R1, the voltage across it is approx. 90% of the source.'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
